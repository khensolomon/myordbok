...
export async function main(req) {
	// env.config.fileName.sqlite = '/dev/lidea/assets/tmp/node-word.db';
	// const sqlite = new sqliteDatabase.Database(env.config.fileName.sqlite);
	// db.serialize(() => {
...
async function createWord() {
	...
	const sqlite = new sqliteDatabase.Database(
		"/dev/lidea/assets/tmp/node-word.db"
	);
	sqlite.serialize(() => {
		sqlite.run("BEGIN");
...
async function listOfWord() {
	return db.mysql.query("SELECT id, word, derived FROM ?? LIMIT 5000;", [
		table.synset
	]);
}
...
async function listOfThesaurus() {
	return db.mysql.query("SELECT wrid, wlid FROM ??;", [table.thesaurus]);
}
