...
exports.testing = async function(param) {
	const data = fs.readFileSync(
		"./test/FSIN_English_Myanmar_Dictionary.csv",
		"UTF-8"
	);
...
const sqlFindWord = async function(word) {
	return await app.sql.query(
		"SELECT word AS term, tid AS pos, sense AS v,exam FROM ?? WHERE LOWER(word) LIKE LOWER(?) ORDER BY tid, seq;",
		[table.senses, word]
	);
};
...
const sqlInsert = async function(val) {
	await app.sql
		.query(
			"INSERT INTO ?? (word, tid, sense, exam, seq, kid) VALUES (?,?,?,?,?,?);",
...
const sqlUpdate = async function(val) {
	...
	await app.sql
		.query(
			"UPDATE ?? SET word=?, tid=?, sense=?, exam=?, seq=?, kid=? WHERE `id`=?;",
			[
				"senses_test",
				val.word,
				val.tid,
...
const sqlGetNullId = async function() {
	// table.senses,row.word
	var raw = await app.sql
		.query("SELECT id FROM ?? WHERE sense IS NULL LIMIT 1;", ["senses_test"])
		.catch(e => console.error(e.message));
	if (raw.length) {
		return raw[0].id;
	}
