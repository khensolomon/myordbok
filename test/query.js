...
describe("Query", () => {
	it("Simple validation: string", () => {
		const script_replaced = check.validate("<script></script>");
		assert.strictEqual("", script_replaced);

		const img_replaced = check.validate("<img></img>");
		assert.strictEqual("", img_replaced);

		const any_img_replaced = check.validate("<img></img><button><script>");
		assert.strictEqual("", any_img_replaced);

		const no_attr_allow = check.validate(
