...
def multiplication(length, head="1"):
    """
...
def keep(s):
    ...
    """
    return "".join([_ASCII_TO_BURMESE_MAP.get(char, char) for char in s])
...
def turn(s):
    ...
    """
    return "".join([_BURMESE_TO_ASCII_MAP.get(char, char) for char in s])

def _chunk(s, size, right=0):
    """
...
def _sense(s="0"):
    ...
    if k <= 1 and int(s) < 1:
        row["sense"] = _CONFIG["numeral"][0]["name"]
        return row
    ...
    for i, char_num in enumerate(s):
        ...
        if num > 0:
            ...
            if raw and index < 3:
                last = raw[-1]
                if "creaky" in _CONFIG["scale"][last["index"]]:
                    last["creaky"] = True
                    last["tone"] = _CONFIG["scale"][last["index"]]["creaky"]
            
            raw.append({
                "name": _CONFIG["numeral"][num]["name"],
                "tone": _CONFIG["scale"][index]["name"],
                "creaky": False,
                "index": index
            })
    ...
    for i, num_chunk in enumerate(list_chunks):
        ...
        if row["sense"]:
            if is_head:
                ...
                if i == last_index:
                    fst += _CONFIG["conjunction"]["space"]
                else:
                    fst += _CONFIG["conjunction"]["and"]
                fst += row["sense"]
                if i != last_index:
                    fst += _CONFIG["conjunction"]["comma"]
...
def get(s):
    ...
    if any(digit in s for digit in _DIGIT_BURMESE):
        s = turn(s)
    ...
    if int(query) <= 0:
        result["notation"].append(_sense('0'))
    else:
        # Filter applicable scales
        applicable_scales = [
            e for e in _CONFIG["scale"] 
            if (e.get("limit") and e["rule"] <= k) or e["rule"] == k
        ]
        result["notation"] = [_engine(query, e) for e in applicable_scales]
